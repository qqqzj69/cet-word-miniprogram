...
备考四六级时需要一个能随手打开、断网也能背、不担心进度丢失的单词工具，于是自己写了一个。

一个**完全离线、零依赖、开箱即用**的微信小程序：基于简化艾宾浩斯记忆曲线安排复习，词库内置，学习数据存在本机。
...
1. 克隆仓库
   ...
   ```
2. 用**微信开发者工具**导入项目根目录
3. 把 `project.config.json` 里的 `appid` 换成你自己的（也可以在导入时选择「测试号」直接体验），编译即可运行
